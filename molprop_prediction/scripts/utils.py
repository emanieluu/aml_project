...
def prompt_user_for_args():
    ...
    params_file = input(
        "Enter the name of the JSON parameter file to load (default: params.json): "
    )
    params_file = params_file.strip() or "params.json"
    saving_name = input(
        "Enter the name of the model (saved in models/trained_models): "
    )
    print(
        f"Training the {model} model with parameters from the file {params_file} and saving it as {saving_name}."
    )
    config_path = f"./molprop_prediction/configs/{model}/{params_file}"
    save_path = f"./molprop_prediction/trained_models/{model}/{saving_name}"
    return model, config_path, save_path


def prompt_user_for_predictions():
    model = input(
        "Which model would you like to use to predict? (RF, GIN, GAT): "
    )
...
def read_tabular_train():
    data = pd.read_csv("./data/tabular_preprocessed_data/train_data.csv")
    return data


def read_tabular_test():
    data = pd.read_csv("./data/tabular_preprocessed_data/test_data.csv")
    return data
...
def load_model(model, optimizer, checkpoint_path):
    ...
    print(
        f"Model loaded from {checkpoint_path}, trained for {epoch} epochs. Last loss: {loss}, Last MAE: {mae}"
    )
...
def translate_params(param_indices):
    ...
    for param, index in param_indices.items():
        if (
            ...
            or param == "k"
            or param == "preprocessor"
            or param == "n_components"
            or param == "whiten"
            or param == "n_features_to_select"
            or param == "step"
        ):
