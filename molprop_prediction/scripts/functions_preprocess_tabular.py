...
def smiles_to_mol(dataset, smiles_column, mol_column):
    ...
    dataset_return[mol_column] = dataset_return[smiles_column].apply(
...
def add_fingerprints_features(dataset, molecule_column):
    ...
    """
    dataset_return = dataset
    dataset_return["fps_1"] = [
        np.array(
            AllChem.GetMorganFingerprintAsBitVect(mol, radius=2, nBits=2048)
        )
        for mol in dataset_return[molecule_column]
    ]
    dataset_return["num_bits_active"] = dataset_return["fps_1"].apply(
        lambda x: np.sum(x)
    )
    dataset_return["mean_bit_density"] = dataset_return["fps_1"].apply(
        lambda x: np.mean(x)
    )
    dataset_expanded = pd.DataFrame(
        dataset_return["fps_1"].tolist(),
        columns=[f"feature_{i}" for i in range(1, 2049)],
    )
...
def add_mol_features(dataset, molecule_column):
    ...
    """
    dataset_return = dataset
    dataset_return[molecule_column] = dataset_return[molecule_column].apply(
        lambda x: Chem.AddHs(x)
    )  # AddHs function adds H atoms to a MOL (as Hs in SMILES are usualy ignored)
    dataset_return["num_of_atoms"] = dataset_return[molecule_column].apply(
        lambda x: x.GetNumAtoms()
    )
    dataset_return["num_of_heavy_atoms"] = dataset_return[
        molecule_column
    ].apply(lambda x: x.GetNumHeavyAtoms())
...
def number_of_atoms(dataset, mol_column, atom_list):
    ...
    dataset_return = dataset
    for i in atom_list:
        dataset_return["num_of_{}_atoms".format(i)] = dataset_return[
            mol_column
        ].apply(lambda x: len(x.GetSubstructMatches(Chem.MolFromSmiles(i))))
    return dataset_return
...
def add_descriptors(dataset, mol_column):
    ...
    """
    descrs = [
        Descriptors.CalcMolDescriptors(mol) for mol in dataset[mol_column]
    ]
